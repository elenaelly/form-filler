(function() {
	...
	CssSelectorGenerator = (function() {
		...
		CssSelectorGenerator.prototype.sanitizeItem = function(item) {
			return escape(item).replace( /([^a-zA-Z0-9])/g, "\\$1" );
		};
		
		CssSelectorGenerator.prototype.validateId = function(id, dom) {
			if (id == null) {
				return false;
			}
			if (/^\d/.exec(id)) {
			...
			if (dom.querySelectorAll("#" + id).length !== 1) {
				return false;
			}
			return true;
		};
		...
		CssSelectorGenerator.prototype.getIdSelector = function(element) {
			...
			id = this.validateId(id, element.getRootNode()) ? id = "#" + id : id = null;
			return id;
		};
		...
		CssSelectorGenerator.prototype.testSelector = function(element, selector) {
			...
			if ((selector != null) && selector !== '') {
				result = element.getRootNode().querySelectorAll(selector);
		...
		CssSelectorGenerator.prototype.testUniqueness = function(element, selector) {
			...
			return found_elements && found_elements.length === 1 && found_elements[0] === element;
		};
